...
f.LoadDepartmentRanks = function( pnl, id, isRefreshing )
    ...
    local ranks = {}
    for _, rankData in pairs( Falcon.Departments ) do
    ...
    for otherID, rank in SortedPairs( Falcon.Departments[id].ranks or {}, true ) do
        local namae = tostring(otherID) .. ". " .. rank.name .. " [" .. rank.abr .. "] [CL " .. rank.clearance .. "]"
        local b = Falcon.UI.Presets.Buttons.CreateCategoricalButton( pnl, 0.9, 0.075, 0, 0, { text = namae, fade = true, font = "F18", paint = function( self, w, h )
            ...
            pnl.CurrentRankID = otherID
        end } )
        b:Dock( TOP )

        if isRefreshing then
            if otherID == pnl.CurrentRankID then
                b.IsActive = true
...
f.CreateNewDepartment = function( parent )
    ...
    local create = Falcon.UI.Presets.Buttons.CreateConditionalButton( fr, function( self )
        ...
        Falcon.Departments[count] = { name = namae, ranks = {} }
        parent:Clear()
