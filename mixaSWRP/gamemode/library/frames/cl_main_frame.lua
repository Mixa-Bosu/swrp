...
f.CreateBaseFrame = function( w, h, x, y, extras )
    ...
    f:MakePopup()
    f.ShouldAnim = extras.shouldAnimate or false
    ...
    if extras.shouldAnimate then
        ...
        f.Think = function( self )
            ...
            if self.ShouldAnim then
                ...
                self:SetAlpha( math.Clamp(alpha + ((FrameTime() * (extras.animSpeed or 5)) * 255), 0, 255) )
            else
                if alpha <= 0 then
                ...
                self:SetAlpha( math.Clamp(alpha - ((FrameTime() * ((extras.animSpeed or 5) * 2)) * 255), 0, 255) )
            end
